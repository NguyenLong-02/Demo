...
{
    public class ATree : MonoBehaviour, IMaterial
    {
        [Header("GAMEOBJECT")]
        ...
        [SerializeField] private GetMaterial GetMaterial_button_enable; // Tiến trình process
        ...
        void FixedUpdate()
        {
            ...
            {
                lifetime = 20f;
                Resousce.Instance.woods += materials;
                Resousce.Instance.twigs += Random.Range(20, 100);
                Resousce.Instance.small_stone += Random.Range(20, 100);
        ...
        public void Get_Mat() // Dùng để tiến tới thu hoặch
        {
            ...
            Notifi_Discription.text = "IS CHOPPING TREES";
            Request.SetActive(false);
            GetMaterial_button_enable.list_button_enable.ForEach((but) => Destroy(but));
            GetMaterial_button_enable.list_button_enable.Clear();
            GetMaterial.interactable = true; // Khi ấn nút để tiến hành Khai thác tài nguyên sẽ đổi khả năng của interactable của GetMaterial
            // Thành true
